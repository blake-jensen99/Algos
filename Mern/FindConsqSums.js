...
function findConsqSums(nums, targetSum) {
    ...
    for(let i = 0; i < nums.length; i++) {
        ...
        while(sum < targetSum) {
            if(sum + nums[idx] < targetSum) {
                newSolution.push(nums[idx]);
                sum += nums[idx];
                // console.log(sum);
                // console.log(newSolution);
                idx++;
            } else if(sum + nums[idx] == targetSum) {
                sum += nums[idx];
                newSolution.push(nums[idx]);
                solutions.push(newSolution);

                const zeroCheck = [...newSolution];
                for(let j = idx + 1; nums[j] == 0; j++) {
                    zeroCheck.push(nums[j]);
                    solutions.push(zeroCheck);
                }
            } else {
                break;
            }
